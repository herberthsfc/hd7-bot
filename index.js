		...
		try {
			...
			switch(command) {
				...
				case 'setprefix':
					...
					prefix = args[0]
					reply(`O prefixo foi alterado com sucesso, Herberth. ✔️ : ${prefix}`)
					break
				case 'loli':
					loli.getSFWLoli(async (err, res) => {
						if (err) return reply('❌ *ERRO* ❌')
						buffer = await getBuffer(res.url)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Comando Concluido com Sucesso✔️'})
					})
					break
				case 'nsfwloli':
				...
				case 'ban':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Comando 100% concluido ✔️ : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'ademirs':
				...
				case 'bemvindo':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Comando 100% concluido')
						welkom.push(from)
